...
public class TrumpTagsWithLossHandle{
	...
	public static class tagCount extends BaseRichBolt{
		...
		public void execute(Tuple tuple){
			...
			if(System.currentTimeMillis() % 10 != 0){
                TrumpTagsWithLossHandle.totalNum++;
			...
			if(System.currentTimeMillis() - startTime > TrumpTagsWithLossHandle.interval){
                ...
                while(it.hasNext()){
					...
					float frequency = (float) entry.getValue() / TrumpTagsWithLossHandle.totalNum;
					if(frequency > 0.01 && frequency < 1){
                        writer.println("Results: " + entry.getKey() + " " + frequency);
                    }
				}
                writer.println("-------------------------------");
                writer.close();
	...
	public static void main(String[] args) throws Exception{
		...
		TrumpTagsWithLossHandle.startTime = System.currentTimeMillis();
		TrumpTagsWithLossHandle.totalNum = 0;
		builder.setBolt("count", new tagCount(), 1).setNumTasks(1).shuffleGrouping("spout");
		...
		if(args  != null && args.length > 1){
		    ...
		    cluster.submitTopology("TrumpTagsWithLossHandle", conf, builder.createTopology());
...
class TwitterSpout extends BaseRichSpout{
	...
	public void open(Map conf, TopologyContext context, SpoutOutputCollector collector){
		...
		_collector = collector;
        try{
            record = new PrintWriter(new FileOutputStream(new File("Records.txt")));
        }
        catch(IOException e){
		...
		StatusListener listener = new StatusListener(){
			...
			public void onStatus(Status status){
				...
				for(HashtagEntity hashtag: hashtagEntities){
					...
					queue.offer(str);
                    record.println(hashtag.getText().toLowerCase());
				}
			}
			@Override
